...
/**
 * Trim function which works with UTF-8 strings
 *
 * By default all 25 Unicode characters definded as whitespace will be removed
 * from the beginning and the end of the given string
 *
 * @param string $str                   String to be trimmed
 * @param array|string $character_mask  Characters to remove from beginning and end of the string
 *
 * @return string
 **/
if (!function_exists('unicode_trim')) {
    function unicode_trim($str, $character_mask = UNICODE_WS) {
        if (empty($str) || empty($character_mask)) {
            return $str;
        }
        ...
        if (is_string($character_mask)) {
            $character_mask = preg_split('//u', $character_mask, -1, PREG_SPLIT_NO_EMPTY);
        }
        $unicode_chars = preg_split('//u', $str, -1, PREG_SPLIT_NO_EMPTY);

        do {
            if (in_array($unicode_chars[$start], $character_mask)) {
                $start++;
            } else {
                break;
            }
        ...
        do {
            if (in_array($unicode_chars[$end], $character_mask)) {
                $end--;
            } else {
                break;
            }
        ...
        if ($start == $end) {
            return $unicode_chars[$start];
        } elseif ($end < $start) {
            return '';
        } elseif ($start != 0 || $end != $len - 1) {
            return mb_substr($str, $start, $end - $start + 1);
        } else {
            return $str;
        }
    }

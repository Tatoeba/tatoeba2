...
class TranscriptionsHelper extends AppHelper
{
        ...
        if ($needsReview && $warn) {
            ...
            if ($transcr['type'] == 'altscript') {
                $warningMessage = __(format(
                    ...
                    '{warningIcon} icon, which means it <strong>may contain '.
                    'errors</strong> because it has been generated by a piece '.
                    'of software that is not perfect.',
                    compact('warningIcon')),
                    true
                );
            } else {
    ...
    private function warningReviewButton($canEdit, $needsReview) {
        ...
        if ($needsReview) {
            ...
            $title = __("May contain errors.", true);
            if ($canEdit) {
                $content .= $this->Images->svgIcon('check', array(
                    'class' => 'check',
                ));
                $title .= "\n" . __('Click to mark as reviewed.', true);
            }
        };

<div ng-repeat="translation in <?= $translations ?>"
    ...
    <icon-with-progress is-loading="vm.iconsInProgress['link' + translation.id]" ng-if="vm.menu.canLink">
        <md-button class="md-icon-button" ng-show="vm.isMenuExpanded && translation.isDirect" ng-click="vm.saveLink('delete', translation)">
            <md-icon md-svg-src="/img/link_off.svg"></md-icon>
            <md-tooltip><?= __('Unlink this translation') ?></md-tooltip>
        </md-button>
        <md-button class="md-icon-button" ng-show="vm.isMenuExpanded  && !translation.isDirect" ng-click="vm.saveLink('add', translation)">
            <md-icon>link</md-icon>
            <md-tooltip><?= __('Make into direct translation') ?></md-tooltip>
        </md-button>
    </icon-with-progress>
    <md-icon class="chevron" ng-show="!vm.isMenuExpanded || !vm.menu.canLink">chevron_right</md-icon>
    </div>
